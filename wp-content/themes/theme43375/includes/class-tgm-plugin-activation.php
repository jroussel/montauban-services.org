...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				'page_title'                      => __( 'Install Required Plugins', CURRENT_THEME ),
				'menu_title'                      => __( 'Install Plugins', CURRENT_THEME ),
				'installing'                      => __( 'Installing Plugin: %s', CURRENT_THEME ),
				'oops'                            => __( 'Something went wrong.', CURRENT_THEME ),
				...
				'return'                          => __( 'Return to Required Plugins Installer', CURRENT_THEME ),
				'plugin_activated'                => __( 'Plugin activated successfully.', CURRENT_THEME ),
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', CURRENT_THEME ),
			);
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				<?php if ( isset( $this->message ) ) _e( wp_kses_post( $this->message ), CURRENT_THEME ); ?>
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', CURRENT_THEME ) . '</a></p>';
				...
				foreach ( $this->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( $this->strings['return'], CURRENT_THEME ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', CURRENT_THEME ) . '">' . __( 'Return to the Dashboard', CURRENT_THEME ) . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf( __( 'The following plugin was activated successfully: %s.', CURRENT_THEME ), '<strong>' . $plugin['name'] . '</strong>' );
				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', CURRENT_THEME ) . '</a>',
						)
					);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', CURRENT_THEME );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', CURRENT_THEME );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', CURRENT_THEME );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', CURRENT_THEME );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', CURRENT_THEME ) : __( 'Recommended', CURRENT_THEME );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', CURRENT_THEME ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', CURRENT_THEME ) );
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', CURRENT_THEME ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', CURRENT_THEME ),
				'source' => __( 'Source', CURRENT_THEME ),
				'type'   => __( 'Type', CURRENT_THEME ),
				'status' => __( 'Status', CURRENT_THEME )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', CURRENT_THEME ),
				'tgmpa-bulk-activate' => __( 'Activate', CURRENT_THEME ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, CURRENT_THEME ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', CURRENT_THEME );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', CURRENT_THEME );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', CURRENT_THEME );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', CURRENT_THEME );
				$this->strings['process_failed']      = __( 'Plugin install failed.', CURRENT_THEME );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', CURRENT_THEME );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', CURRENT_THEME );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', CURRENT_THEME );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', CURRENT_THEME );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', CURRENT_THEME ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', CURRENT_THEME ) . '</span><span class="hidden">' . __( 'Hide Details', CURRENT_THEME ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', CURRENT_THEME );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', CURRENT_THEME );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', CURRENT_THEME );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', CURRENT_THEME );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', CURRENT_THEME );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', CURRENT_THEME ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', CURRENT_THEME ) . '</span><span class="hidden">' . __( 'Hide Details', CURRENT_THEME ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', CURRENT_THEME );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', CURRENT_THEME );
				}
			...
			public function bulk_footer() {
				...
				foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, admin_url( TGM_Plugin_Activation::$instance->parent_url_slug ) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . __( TGM_Plugin_Activation::$instance->strings['return'], CURRENT_THEME ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', CURRENT_THEME ) . '">' . __( 'Return to the Dashboard', CURRENT_THEME ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
